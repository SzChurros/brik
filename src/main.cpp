...
int main (int argc, char* argv[])
{
	...
	if (strcmp(argv[1], "--version") == 0) {
		printf("brik by SzChurros v1.1.0\n");
		return 0;
	}
	...
	printf("set up brik's variables\n");
		...
		if (filecontent[crntln] == "Cpp") {
			...
			for (int i = 0; i < temp; i++) {
				...
				while (getline(file, crnt)) {
					...
					printf("added: %s", crnt.c_str());
				}

				file.close();
			}
		}
		else if (filecontent[crntln] == "flag") {
			crntln++;
			temp = stoi(filecontent[crntln++]);
			for (int i = 0; i < temp; i++)
				flags.push_back(filecontent[crntln++]);
		}
	}
